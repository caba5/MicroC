...
let process_source filename =
  ...
  try
    let llmodule =
      ...
      Parsing.parse filename |> Semantic_analysis.type_check
      |> Codegen.to_llvm_module
      (* let x = Parsing.parse lexbuf in
         let y = Semantic_analysis.type_check x in
         let z = Ast.show_program y in
         Printf.printf "\n\n%s\n\n" z;
         Codegen.to_llvm_module y; *)
    in
